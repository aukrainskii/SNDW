...
function ABdevDND_ul_dd_shortcode( $attributes, $content = null ) {
	...
	return '<ul class="dnd_shortcode_ul '.esc_attr($class.$animation_class).'"'.$animation_animation.$animation_duration.$animation_delay.'>'.do_shortcode($content).'</ul>';
}
...
function ABdevDND_li_dd_shortcode( $attributes, $content = null ) {
	...
	$icon_out = ($icon!='') ? '<i class="'.esc_attr($icon).'"'.esc_attr($icon_color_out).'></i> ' : '';
