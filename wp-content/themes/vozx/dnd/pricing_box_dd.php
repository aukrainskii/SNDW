...
function ABdevDND_pricing_box_dd_shortcode( $attributes, $content = null ) {
	...
	$decsription_output=($decsription!='')?'<span class="dnd_pricebox_decsription">'.wp_kses($decsription, ABdev_allowed_tags()).'</span>':'';
	$featured_text_output=($featured_text!='')?'<div class="dnd_pricebox_featured_text">'.wp_kses($featured_text, ABdev_allowed_tags()).'</div>':'';
	...
	<div class="dnd_pricing-table-'.esc_attr($style).' dnd_pricing-table-'.esc_attr($header_color).' '.esc_attr($featured_output).' '.esc_attr($class).'">
		<div class="dnd_plan dnd_plan'.esc_attr($count_priceboxes.$featured_output).'">
...
function ABdevDND_pricing_feature_dd_shortcode( $attributes, $content = null ) {
	...
	$yes_output = ($yes=='1')?'<i class="ci_icon-check"></i>':'';
	$no_output = ($no=='1')?'<i class="ci_icon-times"></i>':'';
