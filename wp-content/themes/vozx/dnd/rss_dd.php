...
function ABdevDND_rss_dd_shortcode($attributes) {
	...
	if ($maxitems == 0){
		...
		foreach ( $rss_items as $item ) 
			$return.='<li><a href="'. esc_url( $item->get_permalink() ).'" title="'.esc_attr__('Posted','dnd-shortcodes').' '.esc_attr($item->get_date('j F Y | g:i a')).'"'.esc_attr($target_output).'>'.esc_html( $item->get_title() ).'</a></li>';
	}
	$return.='</ul>';
