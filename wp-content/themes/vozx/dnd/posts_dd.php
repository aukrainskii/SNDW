	...
	function ABdevDND_posts_dd_shortcode( $attributes ) {
		...
		while ( $listing->have_posts() ): $listing->the_post(); 
			...
			$output_date2 = '<div class="date_container">
								<div class="dnd_posts_date"><i class="ci_icon-clock-o"></i> ' . get_the_date('d F') . '</div>
			...
			$output .= '<div class="dnd_posts_shortcode dnd_posts_shortcode-'.esc_attr($style).' clearfix'.$hasthumb_class.'">';
			...
			$output .= '<h5><a href="' . esc_attr(get_permalink()) . '">' . get_the_title() . '</a></h5>';
