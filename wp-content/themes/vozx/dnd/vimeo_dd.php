...
function ABdevDND_vimeo_dd_shortcode($attributes) {
	...
	$video_src='http://player.vimeo.com/video/'.esc_attr($id).'?title='.esc_attr($title).'&amp;byline='.esc_attr($byline).'&amp;portrait='.esc_attr($portrait).'&amp;autoplay='.esc_attr($autoplay).'&amp;loop='.esc_attr($loop).'&amp;color='.esc_attr($color);

	return '<div class="dnd-videoWrapper-vimeo"><iframe src="'.esc_url($video_src).'" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';
}
