...
function ABdevDND_map_dd_shortcode( $attributes ) {
	...
	$map_data = '
		data-map_type="'.esc_attr($map_type).'" 
		data-lat="'.esc_attr($lat).'" 
		data-lng="'.esc_attr($lng).'" 
		data-zoom="'.esc_attr($zoom).'" 
		data-scrollwheel="'.esc_attr($scrollwheel).'" 
		data-maptypecontrol="'.esc_attr($maptypecontrol).'" 
		data-pancontrol="'.esc_attr($pancontrol).'" 
		data-zoomcontrol="'.esc_attr($zoomcontrol).'" 
		data-scalecontrol="'.esc_attr($scalecontrol).'" 
		data-markertitle="'.esc_attr($markertitle).'" 
		data-markericon="'.esc_attr($markericon).'" 
		data-markercontent="'.esc_attr($markercontent).'" 
		data-markerlat="'.esc_attr($markerlat).'" 
		data-markerlng="'.esc_attr($markerlng).'" 
	';

	return '<div id="dnd_google_map_'.esc_attr($i).'" '.$map_data.' class="dnd_google_map" style="height:'.esc_attr($h).';width:100%;"></div>';
}
