...
function ABdevDND_divider_dd_shortcode( $attributes ) {
	...
	$icon_output=($icon=='1')?' <i class="ci_icon-chevron-up"></i>':'';
	$divider_style = ($style != '') ? 'dnd_divider_'.esc_attr($style).'' : '';
	...
	return '<div class="dnd_divider '.esc_attr($divider_style).'"><a href="#" class="backtotop'.esc_attr($animation_class).'"'.esc_attr($animation_animation).esc_attr($animation_duration).'>'.$text.$icon_output.'</a></div>';
}
