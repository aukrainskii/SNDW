...
function ABdevDND_icon_dd_shortcode( $attributes, $content = null ) {
	...
	if($color!='' || $size!=''){
		...
		$icon_style_output=' style="'.esc_attr($color_output.$size_output).'"';
	}
	...
	if(($padding!='' || $background!='') && $box=='1'){
		...
		$box_style_output_before='<span style="'.esc_attr($padding_output.$background_output).' display:inline-block;" class="'.esc_attr($class).'">';
		$box_style_output_after='</span>';
	}
	return $box_style_output_before.'<i class="'.esc_attr($name.$rotate_output).'"'.esc_attr($icon_style_output).'></i>'.$box_style_output_after;
}
