			...
			$return .= '<ul class="ab-tweet-scroller-inner">';
			 	...
			 	foreach ($tweets as $tweet) {
						$return.='<li class="ab-tweet-item">';
							...
							$return .= (!$hide_image) ? '<a class="ab-tweet-image" href="https://twitter.com/'.esc_attr($user).'" target="'.esc_attr($link_target).'"><img src="'.esc_url($tweet->user->profile_image_url_https).'" class="imgalign"></a>' : '';
							
							$return .= (!$hide_username) ? '<a class="ab-tweet-username" href="https://twitter.com/'.esc_attr($user).'" target="'.esc_attr($link_target).'">'.$tweet->user->screen_name.'</a>' : '';
							...
							$return .= ($date_out!='') ? '<a class="ab-tweet-date" href="https://twitter.com/'.esc_attr($user).'" target="'.esc_attr($link_target).'">'.$date_out.'</a>' : '';
							
							$return .= '<div class="ab-tweet-text">'.ab_make_links_clickable($tweet->text, $link_target).'</div>';

							$links_out = '';
							$links_out .= (!$hide_reply) ? '<a class="ab-tweet-links-reply" target="'.esc_attr($link_target).'" href="https://twitter.com/intent/tweet?in_reply_to='.$tweet->id_str.'">'.__("Reply", 'ab-tweet-scroller').'</a>' : '';
							$links_out .= (!$hide_retweet) ? '<a class="ab-tweet-links-retweet" target="'.esc_attr($link_target).'" href="https://twitter.com/intent/retweet?tweet_id='.$tweet->id_str.'">'.__("Retweet", 'ab-tweet-scroller').'</a>' : '';
							$links_out .= (!$hide_favorite) ? '<a class="ab-tweet-links-favorite" target="'.esc_attr($link_target).'" href="https://twitter.com/intent/favorite?tweet_id='.$tweet->id_str.'">'.__("Favorite", 'ab-tweet-scroller').'</a>' : '';
