	...
	switch ($column){
		...
		case "name":
			if ( get_post_status () == 'trash' ) {
				echo '<strong>'.esc_html($custom['ABt_testimonial_client'][0]). '</strong><div class="row-actions"><span class="trash"><a href="'. esc_url(abdev_get_undelete_post_link( $post->ID )) .'">'.__('Restore', 'ABt').'</a></span></div>';
			} else {
				echo '<a href="'. esc_url(get_edit_post_link( $post->ID )) .'"><strong>'. esc_html($custom['ABt_testimonial_client'][0]). '</strong></a><div class="row-actions"><span class="edit"><a href="'. esc_url(get_edit_post_link( $post->ID )) .'">'.__('Edit', 'ABt').'</a></span> | <span class="trash"><a href="'. esc_url(get_delete_post_link( $post->ID ) ).'">'.__('Trash', 'ABt').'</a></span></div>';
			}
		break;
		case "cat":
...
function ABt_testimonials_manager_meta_options(){  
	...
	<div class="testimonials_manager_extras">
		<p>
			...
			<input name="ABt_testimonial_client" value="<?php echo esc_attr($ABt_testimonial_client); ?>" />
		</p>
		<p>
			<label><?php _e('Client URL:', 'ab-testimonials');?></label>
			<input name="ABt_testimonial_client_url" value="<?php echo esc_attr($ABt_testimonial_client_url); ?>" />
		...
		<p>
			...
			<input name="ABt_testimonial_company" value="<?php echo esc_attr($ABt_testimonial_company); ?>" />
		</p>
		<p>
			<label><?php _e('Company URL:', 'ab-testimonials');?></label>
			<input name="ABt_testimonial_company_url" value="<?php echo esc_attr($ABt_testimonial_company_url); ?>" />
		...
		<p>
			...
			<textarea name="ABt_testimonial_text"><?php echo esc_attr($ABt_testimonial_text); ?></textarea>
		</p>
