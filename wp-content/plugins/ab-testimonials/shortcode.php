...
function ABt_testimonials_shortcode($atts, $content){
	...
	if ($post->have_posts()){
		while ($post->have_posts()){
			...
			if($ABt_testimonial_client!=''){
				$author_out = ($ABt_testimonial_client_url != '') ? '<a class="ABt_author" href="'.esc_url($ABt_testimonial_client_url).'" target="'.esc_attr($ABt_testimonial_client_url_target).'">'.wp_kses($ABt_testimonial_client, ABdev_allowed_tags()).'</a>' : '<span class="ABt_author">'.wp_kses($ABt_testimonial_client, ABdev_allowed_tags()).'</span>';
			}
			if($ABt_testimonial_company!=''){
				$company_out = ($ABt_testimonial_company_url != '') ? '<a class="ABt_company" href="'.esc_url($ABt_testimonial_company_url).'" target="'.esc_attr($ABt_testimonial_company_url_target).'">'.wp_kses($ABt_testimonial_company, ABdev_allowed_tags()).'</a>' : '<span class="ABt_company">'.wp_kses($ABt_testimonial_company, ABdev_allowed_tags()).'</span>';
			}
			$out.= '
				<li class="testimonials_item">
					<div class="'.esc_attr($type_class).'">
						<p>' . $ABt_testimonial_text . '</p>
						'.$picture_out.'
						<span class="source">' . $author_out . ' ' . wp_kses($delimiter, ABdev_allowed_tags()) . ' ' . $company_out . '</span>
					</div>
				</li>';
		}
	}
	...
	return '
		<div class="'. $wrapper_class .' '. esc_attr($class) .' '.$picture_position.'">
			'. $show_arrows_out .'
...
function ABt_submit_form($atts, $content){
	extract(shortcode_atts(array( 
		...
		return '
			<div class="ABt_form_wrapper '.esc_attr($class).'">
				<form id="' . esc_attr($form_id) . '" class="ABt_form" action="#" method="post">
					<input type="text" name="ABt_client" class="ABt_client" placeholder="'.esc_attr($client_placeholder).'">
					<input type="text" name="ABt_client_url" class="ABt_client_url" placeholder="'.esc_attr($client_url_placeholder).'">
					<input type="text" name="ABt_company" class="ABt_company" placeholder="'.esc_attr($company_placeholder).'">
					<input type="text" name="ABt_company_url" class="ABt_company_url" placeholder="'.esc_attr($company_url_placeholder).'">
					<textarea name="ABt_text" class="ABt_text" placeholder="'.esc_attr($text_placeholder).'"></textarea>
					<label>'.esc_attr($client_image_placeholder).'</label>
					<input type="file" name="ABt_client_image" class="ABt_client_image">
					<p><input type="submit" value="'.esc_attr($button_text_placeholder).'"></p>
					<input type="hidden" name="ajaxnonce" value="' . wp_create_nonce( $form_id ) . '">
					<input type="hidden" name="formno" value="' . esc_attr($form_number) . '">
