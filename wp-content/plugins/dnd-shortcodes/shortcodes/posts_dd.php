...
if ( ! function_exists( 'ABdevDND_posts_dd_shortcode' ) ){
	function ABdevDND_posts_dd_shortcode( $attributes ) {
		...
		while ( $listing->have_posts() ): $listing->the_post(); 
			...
			$output .= '<div class="dnd_posts_shortcode clearfix'.esc_attr($hasthumb_class).'">';
			$output .= ($thumbnail!='') ? '<a class="dnd_latest_news_shortcode_thumb" href="' . esc_url(get_permalink()) . '">' . get_the_post_thumbnail($post->ID,'thumbnail') . '</a>' :'';
