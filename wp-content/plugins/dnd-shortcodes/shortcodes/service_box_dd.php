...
function ABdevDND_service_box_dd_shortcode( $attributes, $content = null ) {
	...
	$return = '
		<div class="dnd_service_box'.esc_attr($class).'">
			<div class="dnd_service_box_header">';

			$return .= ($link!='') ? '<a href="'.esc_url($link).'" target="'.esc_attr($target).'" class="dnd_icon_boxed"><i class="'.esc_attr($icon).'"></i></a>' : '<div class="dnd_icon_boxed"><i class="'.esc_attr($icon).'"></i></div>';
