	...
	function ABdev_generate_attr_fields($attributes, $load_data = array()) {
		...
		foreach ( $attributes as $attribute => $attribute_data ) {
			...
			if($type!='column' || ($type=='column' && $attribute!='span')){
				$return .= '<tr>';
				$info_output = (isset($attribute_data['info'])) ? ' title="'.esc_attr($attribute_data['info']).'"' : '';
				$info_class_output = (isset($attribute_data['info'])) ? 'dnd_attribute_with_info' : '';
				$return .= '<td class="dnd_with_label"><label class="'.$info_class_output.'" for="dnd_shortcode_attribute_'.esc_attr($attribute).'"'.$info_output.'>'.$attribute_data['description'].'</label></td>';
				$attribute_type = (isset($attribute_data['type'])) ? $attribute_data['type'] : '';
				switch ($attribute_type) {
					case "checkbox":
						$return .= "<td><input type='checkbox' name='".esc_attr($attribute)."' value='1' id='dnd_shortcode_attribute_".esc_attr($attribute)."' class='dnd_shortcode_attribute' ".checked($attribute_value, 1, false)."/></td>";
						break;
					case "textarea":
						$return .= "<td><textarea name='".esc_attr($attribute)."' id='dnd_shortcode_attribute_".esc_attr($attribute)."' class='dnd_shortcode_attribute'>".$attribute_value."</textarea></td>";
						break;
					case "select":
						$return .= "<td><select name='".esc_attr($attribute)."' id='dnd_shortcode_attribute_".esc_attr($attribute)."' class='dnd_shortcode_attribute'>";
						foreach ( $attribute_data["values"] as $option_value => $option_name ) {
							$return .= "<option value='".esc_attr($option_value)."' ".selected($attribute_value, $option_value, false).">".$option_name."</option>";
						}
						$return .= "</select></td>";
						break;
					case "color":
						$return .= "<td><input type='text' name='".esc_attr($attribute)."' value='".esc_attr($attribute_value)."' class='dnd_shortcode_attribute dnd-colorpicker' data-default-color='".esc_attr($attribute_info_default)."'></td>";
						break;
					case "image":
						$return .= "<td><input type='text' name='".esc_attr($attribute)."' value='".esc_attr($attribute_value)."' class='dnd_shortcode_attribute' data-default-color='".esc_attr($attribute_value)."'>
					...
					case "media":
						...
						break;
					default:
						$return .= "<td><input type='text' name='".esc_attr($attribute)."' value='".esc_attr($attribute_value)."' id='dnd_shortcode_attribute_".esc_attr($attribute)."' class='dnd_shortcode_attribute'></td>";
						break;
				}
				$return .= '</tr>';
			}
