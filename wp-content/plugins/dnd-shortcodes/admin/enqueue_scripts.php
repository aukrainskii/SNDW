...
function ABdevDND_enqueue_admin_scripts($hook) {
	...
	wp_enqueue_script('dnd-shortcodes-mCustomScrollbar', DND_SHORTCODES_DIR.'js/jquery.mCustomScrollbar.js', array('jquery'), DND_VERSION);
	wp_enqueue_script('dnd-shortcodes-cookie', DND_SHORTCODES_DIR.'js/jquery.cookie.js', array('jquery'), DND_VERSION);
	wp_enqueue_script('dnd-shortcodes-ddtab', DND_SHORTCODES_DIR.'js/ddtab.js', array('dnd-shortcodes-mCustomScrollbar', 'wp-color-picker','jquery-ui-sortable','jquery-ui-resizable','dnd-shortcodes-fancybox'), DND_VERSION,true);
	wp_localize_script('dnd-shortcodes-ddtab', 'dnd_from_WP', array(
		...
		'classis_editor' => __('Classic', 'dnd-shortcodes'),
...
function ABdevDND_enqueue_frontend_script() {
	...
	wp_enqueue_script( 'countdown', DND_SHORTCODES_DIR.'/js/jquery.countdown.js', array( 'jquery' ), DND_VERSION, true );
	...
	$dnd_custom_map_style = (isset($options['dnd_custom_map_style'])) ? $options['dnd_custom_map_style'] : '';

	wp_enqueue_script('dnd-shortcodes', DND_SHORTCODES_DIR.'js/init.js', array('jquery', 'jquery-ui-accordion', 'jquery-ui-tabs', 'jquery-effects-slide', 'animo', 'google_maps_jquery', 'parallax', 'inview' , 'tipsy' , 'knob' , 'prettify', 'countdown'), DND_VERSION, true);
	wp_localize_script( 'dnd-shortcodes', 'dnd_options', array( 
		'dnd_tipsy_opacity' => $dnd_tipsy_opacity, 
		'dnd_custom_map_style' => preg_replace('!\s+!', ' ', str_replace(array("\n","\r","\t"), '', $dnd_custom_map_style)), 
	) );
}
 ...
 */
function ABdevDND_mce_external_plugins($plugins) {   

    $plugins['code'] = DND_SHORTCODES_DIR . 'js/mce.plugin.code.min.js';
    $plugins['link'] = DND_SHORTCODES_DIR . 'js/mce.plugin.link.min.js';
    $plugins['hr'] = DND_SHORTCODES_DIR . 'js/mce.plugin.hr.min.js';
    $plugins['charmap'] = DND_SHORTCODES_DIR . 'js/mce.plugin.charmap.min.js';
    $plugins['searchreplace'] = DND_SHORTCODES_DIR . 'js/mce.plugin.searchreplace.min.js';
    $plugins['table'] = DND_SHORTCODES_DIR . 'js/mce.plugin.table.min.js';
    $plugins['textcolor'] = DND_SHORTCODES_DIR . 'js/mce.plugin.textcolor.min.js';
    return $plugins;
}
add_filter('mce_external_plugins', 'ABdevDND_mce_external_plugins');